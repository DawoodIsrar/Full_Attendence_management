          ...
          {currentRecords.map((row, index) => (
            <tr key={index} className="border-b hover:bg-gray-50 text-center">
              ...
              <td className="px-6 py-4 whitespace-nowrap text-sm  text-gray-900 ">
                <Link to={`/slips/${Data[index]["Employee ID"]}`}>
                  <button className='text-white bg-gradient-to-r from-blue-400 via-blue-500 to-blue-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-3 py-2 text-center mr-2 mb-2'>Current Month Slip</button>
                </Link>
              </td>
              <td className="px-6 py-4 whitespace-nowrap text-sm  text-gray-900 ">
                <Modal index={index} Data={Data} setData={setData} name={Data[index]['Employee Name']} date={Data[index][month]} link={link} />
              </td>
            </tr>
          ))}
        </tbody>
